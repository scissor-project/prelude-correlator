...
class TimestampsWeakWindowHelper(ContextHelper):
    ...
    class _WindowExpirationCache(object):
        ...
        def __init__(self, ctx, origTime, received, startSendTimestamp):
            self._ctx = ctx
    ...
    def rst(self):
        self._origTime = time.time()
        self._startSendTimestamp = self._startSendTimestamp
        self._received = 0

    def setOrigTime(self, or_t):
        self._origTime = or_t

    def setStartSendTimestamp(self, idmef):
        self._startSendTimestamp = self.getSendTimestamp(idmef) \
        if idmef is not None else None
    ...
    def isLocalGap(self):
        return time.time() - \
    self._origTime >= self._ctx.getOptions()["window"]*1000
        ...
        if self.isWindowEnd():
            if self._ctx.getOptions()["reset_ctx_on_window_expiration"]:
                if "check_on_window_expiration" in self._ctx.getOptions() and self._ctx.getOptions()["check_on_window_expiration"]:
                    ...
                    self._received, self._startSendTimestamp)
                self._ctx.destroy()
    ...
    def checkCorrelation(self):
        if "check_on_window_expiration" in self._ctx.getOptions() and self._ctx.getOptions()["check_on_window_expiration"]:
            now_1 = time.time()
            now_2 = self._startSendTimestamp
            ot_1 = self._origTime
            ...
            if self.isDifferenceBetween(now_1, ot_1) or \
